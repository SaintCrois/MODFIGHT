...
export class StatusBar {
    ...
    this.koAnimationTimer = 0;
      ...
      this.frames = new Map([
        ...
        ['tag-natasha', [179, 193, 67, 9]],
        ['tag-jumpol', [257, 194, 56, 9]],  
      ]);
    ...
    checkGameOver() {
      const [player1Health, player2Health] = this.healthBars.map(bar => bar.hitPoints);
  
      if (player1Health <= 0 || player2Health <= 0) {
          // ระบุผู้ชนะ
          const winner = player1Health <= 0 ? 'Player 2 Wins!' : 'Player 1 Wins!';
  
          // แสดงหน้า Game Over
          document.getElementById('game-screen').style.display = 'none';
          const gameOverScreen = document.getElementById('game-over-screen');
          gameOverScreen.style.display = 'block';
          gameOverScreen.querySelector('h1').textContent = winner;
    ...
    drawFrame(context, frameKey, x, y, direction = 1) {
      ...
      drawNameTags(context) {
        ...
        this.drawFrame(context, name2, 298, 33);
      }
      drawTime(context) {
